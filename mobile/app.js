...
const points = replay.map(p => ({
  ...
  speed: Number.isFinite(Number(p.speed)) ? Number(p.speed) : Number(p.speedMps || Number(p.speedKmh || 0) / 3.6),
...
function haversineM(aLat,aLon,bLat,bLon){const R=6371000,r=Math.PI/180,dLat=(bLat-aLat)*r,dLon=(bLon-aLon)*r;const a=Math.sin(dLat/2)**2+Math.cos(aLat*r)*Math.cos(bLat*r)*Math.sin(dLon/2)**2;return 2*R*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));}
function propagate(lat,lon,speedMps,headingDeg,dt){const R=6378137,d=Math.max(0,speedMps)*Math.max(0,dt),b=headingDeg*Math.PI/180,latR=lat*Math.PI/180;return [lat+(d*Math.cos(b)/R)*180/Math.PI,lon+(d*Math.sin(b)/(R*Math.max(Math.cos(latR),1e-6)))*180/Math.PI];}
...
function tick(){
  if(!points.length)return;const p=points[frame],prev=points[Math.max(0,frame-1)],dt=frame===0?0.1:Math.max(0.01,p.t-prev.t||0.1);distance+=p.speed*dt;
  if(!outage){drLat=p.lat;drLon=p.lon;}else if(drLat!==null)[drLat,drLon]=propagate(drLat,drLon,p.speed,p.heading,dt);
  ...
  els.speed.textContent=(p.speed*3.6).toFixed(1);els.heading.textContent=Math.round(p.heading);els.distance.textContent=Math.round(distance);els.drift.textContent=outage?haversineM(drLat,drLon,p.lat,p.lon).toFixed(1):'0.0';els.message.textContent=outage?'GNSS unavailable — speed + course replay is propagating the DR estimate.':'GNSS aiding active — replay position is available.';draw();frame=(frame+1)%points.length;
}
...
els.start.onclick=start;els.outage.onclick=()=>{if(!points.length)return;outage=!outage;const p=points[Math.min(frame,points.length-1)];drLat=p.lat;drLon=p.lon;setMode();draw()};els.reset.onclick=reset;resize();reset();
